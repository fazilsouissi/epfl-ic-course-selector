...
const IndividualCourse = ({courseName, courseInfos, onClick, layout = 'default'}) => {
  ...
  const getBlockBadge = (block, category) => {
    const badges = {
      'A': { label: 'A', class: 'bg-blue-100 text-blue-800 dark:bg-blue-500/20 dark:text-blue-300 border-blue-400 dark:border-blue-500/30' },
      'B': { label: 'B', class: 'bg-green-100 text-green-800 dark:bg-green-500/20 dark:text-green-300 border-green-400 dark:border-green-500/30' },
      'C': { label: 'C', class: 'bg-purple-100 text-purple-800 dark:bg-purple-500/20 dark:text-purple-300 border-purple-400 dark:border-purple-500/30' },
      'Options': { label: 'Opt', class: 'bg-orange-100 text-orange-800 dark:bg-orange-500/20 dark:text-orange-300 border-orange-400 dark:border-orange-500/30' },
      'Physics/Bio': { label: 'Bio', class: 'bg-pink-100 text-pink-800 dark:bg-pink-500/20 dark:text-pink-300 border-pink-400 dark:border-pink-500/30' },
      'Project': { label: 'Proj', class: 'bg-cyan-100 text-cyan-800 dark:bg-cyan-500/20 dark:text-cyan-300 border-cyan-400 dark:border-cyan-500/30' },
      'SHS': { label: 'SHS', class: 'bg-amber-100 text-amber-800 dark:bg-amber-500/20 dark:text-amber-300 border-amber-400 dark:border-amber-500/30' },
      'Hors-plan': { 
        label: category === 'HEC' ? 'HEC' : category === 'Master IC' ? 'Master' : category === 'Autre section' ? 'Autre' : 'HP', 
        class: 'bg-red-100 text-red-800 dark:bg-red-500/20 dark:text-red-300 border-red-400 dark:border-red-500/30' 
      }
    };
  ...
  const getLayoutClasses = () => {
    const baseContainer =
      "group relative flex items-center justify-between p-2.5 mb-2 rounded-lg border border-border/50 bg-background/80 hover:bg-background hover:border-primary/30 cursor-grab active:cursor-grabbing transition-all duration-200 shadow-sm hover:shadow-md select-none " +
      "dark:bg-card/90 dark:hover:bg-card dark:border-border/70 dark:ring-1 dark:ring-white/10 dark:shadow-black/30 dark:hover:shadow-black/40";
    const baseName = "flex-1 font-medium text-sm truncate mr-3 text-foreground/90 group-hover:text-primary transition-colors";
    const baseCredits = "font-bold text-xs whitespace-nowrap text-foreground dark:text-primary bg-primary/10 px-2 py-1 rounded-md border border-primary/20";
