...
const IndividualCourse = ({courseName, courseInfos, onClick, layout = 'default'}) => {
  ...
  const getBlockBadge = (block, category) => {
    const badges = {
      'A': { label: 'A', class: 'bg-blue-100 text-blue-900 dark:bg-blue-500/20 dark:text-blue-300 border-blue-400 dark:border-blue-500/30' },
      'B': { label: 'B', class: 'bg-green-100 text-green-900 dark:bg-green-500/20 dark:text-green-300 border-green-400 dark:border-green-500/30' },
      'C': { label: 'C', class: 'bg-purple-100 text-purple-900 dark:bg-purple-500/20 dark:text-purple-300 border-purple-400 dark:border-purple-500/30' },
      'Options': { label: 'Opt', class: 'bg-orange-100 text-orange-900 dark:bg-orange-500/20 dark:text-orange-300 border-orange-400 dark:border-orange-500/30' },
      'Physics/Bio': { label: 'Bio', class: 'bg-pink-100 text-pink-900 dark:bg-pink-500/20 dark:text-pink-300 border-pink-400 dark:border-pink-500/30' },
      'Project': { label: 'Proj', class: 'bg-cyan-100 text-cyan-900 dark:bg-cyan-500/20 dark:text-cyan-300 border-cyan-400 dark:border-cyan-500/30' },
      'SHS': { label: 'SHS', class: 'bg-amber-100 text-amber-900 dark:bg-amber-500/20 dark:text-amber-300 border-amber-400 dark:border-amber-500/30' },
      'Hors-plan': { 
        label: category === 'HEC' ? 'HEC' : category === 'Master IC' ? 'Master' : category === 'Autre section' ? 'Autre' : 'HP', 
        class: 'bg-red-100 text-red-900 dark:bg-red-500/20 dark:text-red-300 border-red-400 dark:border-red-500/30' 
      }
    };
